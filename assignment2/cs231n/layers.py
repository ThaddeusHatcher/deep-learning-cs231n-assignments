...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    x, x_norm, mean_x, var_x, gamma, beta = cache
    dx_norm = dout * gamma
    dvar_x = np.sum(dx_norm * (x - mean_x), axis=0) * -0.5*pow(var_x, -(3/2))
    dmean_x = (np.sum(dx_norm * (-1 / np.sqrt(var_x)), axis=0)) + dvar_x * np.mean(-2*(x - mean_x)) 
    dx = dx_norm * (1 / np.sqrt(var_x)) + dvar_x * (2*(x - mean_x) / x.shape[0]) + (dmean_x / x.shape[0])
        ...
        for f in range(F):
            for i in range(0, H_prime):
                for j in range(0, W_prime):
                    ...
                    j1 = j * stride
                    j2 = j * stride + HH
                    x_sub = x_pad[:, i1:i2, j1:j2]
                    out[n, f, i, j] = np.sum(x_sub * w[f]) + b[f]
        ...
        for f in range(F):
            for i in range(H_prime):
                for j in range(W_prime):
                    ...
                    dx_pad[:,i1:i2,j1:j2] += dout[n,f,i,j] * w[f]
        dx[n,:,:,:] = dx_pad[:,1:-1,1:-1]
    ...
    for n in range(N):
        for i in range(H_prime):
            for j in range(W_prime):
                ...
                x_sub = x[n, :, i1:i2, j1:j2]
                z = np.max(x_sub, axis=(1,2))
                out[n, :, i, j] = np.max(x_sub, axis=(1,2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
    ...
    for n in range(N):
        for i in range(H_prime):
            for j in range(W_prime):
                ...
                j2 = j * stride + pool_width
                for c in range(C):
                    x_sub = x[n, c, i1:i2, j1:j2]
                    max_vals = np.max(x_sub, axis=(0, 1))
                    dx[n, c, i1:i2, j1:j2] += (max_vals == x_sub) * dout[n, c, i, j] 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_t = np.reshape(np.transpose(x, (0, 2, 3, 1)), (-1, C))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_t = np.reshape(np.transpose(dout, (0, 2, 3, 1)), (-1, C))
